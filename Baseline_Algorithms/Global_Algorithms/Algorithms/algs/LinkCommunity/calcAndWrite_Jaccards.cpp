...
int main (int argc, char const *argv[]){
    ...
    gettimeofday(&end,NULL);  // add
    double span = end.tv_sec-start.tv_sec + (end.tv_usec-  start.tv_usec)/1000000.0;   // added
    ofstream outfile;   // added
    outfile.open("./LCtime.txt", ios::out);   // added
    outfile<<span<<endl;  // added
    outfile.close();    // added
