     ...
     /* normalized the adjacency matrix for Ncut only, D^-.5*A*D^-.5*/
{
  ...
  if (cutType == RASSO){ // ratio asso.
	...
	for (j=xadj[i]; j<xadj[i+1]; j++) 
	  if (w[i]>0)
	    m_adjwgt[j] = 1.0/sqrt((long double)w[i]);
      for (i=0; i<nvtxs; i++)
	for (j=xadj[i]; j<xadj[i+1]; j++) 
	  if (w[adjncy[j]]>0)
	    m_adjwgt[j] /=  sqrt((long double)w[adjncy[j]]);
    }
    else{
      for (i=0; i<nvtxs; i++)
	for (j=xadj[i]; j<xadj[i+1]; j++) 
	  if (w[i]>0)
	    m_adjwgt[j] = adjwgt[j] *1.0/sqrt((long double)w[i]);
	  else
	    m_adjwgt[j] = 0;
      for (i=0; i<nvtxs; i++)
	for (j=xadj[i]; j<xadj[i+1]; j++) 
...
void Weighted_kernel_k_means(CtrlType *ctrl, GraphType *graph, int nparts, idxtype *w, float *tpwgts, float ubfactor){
  ...
  // added by Joyce
  IFSET(ctrl->dbglvl, DBG_COARSEN, printf("sigma value (wkkm.cpp) in matlab folder : %f\n",sigma));
