...
dataPath = '../Original_Data/youtube/';
savePath = '../Processed_Data/';

% load com-youtube.ungraph.txt
ungraph = importdata([dataPath,'com-youtube.ungraph.txt']);
edges = ungraph.data;
...
dlmwrite([savePath,'youtube'],[colId,rowId],'precision',10,'delimiter','\t');

% load com-youtube.top5000.cmty.txt
fid1 = fopen([dataPath,'com-youtube.top5000.cmty.txt'],'r');
cid1 = 1;
...
while 1
    ...
    if cid1 == 1
        dlmwrite([savePath,'youtube5000.txt'],newid','precision',10,'delimiter','\t');
    else
        dlmwrite([savePath,'youtube5000.txt'],newid','precision',10,'-append','delimiter','\t');
    end
    cid1 = cid1 + 1;
end
...
% load com-youtube.all.cmty.txt
fid2 = fopen([dataPath,'com-youtube.all.cmty.txt'],'r');
cid2 = 1;
...
while 1
    ...
    [~,newid,~] = intersect(nonZeroIdx,id);  % remove isolated vertices in each community
    if cid2 == 1
        dlmwrite([savePath,'youtubeAll.txt'],newid','precision',10,'delimiter','\t');
    else
        dlmwrite([savePath,'youtubeAll.txt'],newid','precision',10,'-append','delimiter','\t');
    end
    cid2 = cid2 + 1;
end
